...
| | 언제 막나 | 무엇 |
|---|---|---|
| **막는다** | 커밋 시점부터 | 조용히 틀리면 위험하고 **고치는 데 한 줄**이면 되는 것 — 앵커 · MCP tool 집합 · 안전 결함 인계 · 루트 분량 |
| **알린다** | **PR 에서만** (`--strict`) | 알면 되는 것 — spec 절 번호 · 유령 항목 · 자동 로딩 총량 · `.gitignore` 경로 · 문서 경로(라우팅 문서 포함) · 스키마 파일 미등재 · 자식 `CLAUDE.md` 부재 |
| **말만 한다** | 커밋·CI 에서 | 막지도 미루지도 않는다 — `상태` 줄 넛지(훅) · 보호 문서 순삭제(CI) |
...
`docs/decisions.md` 에 한 줄 남기고 PR 을 열면 된다. 규칙을 지키느라 우회하는 것보다 낫다.
