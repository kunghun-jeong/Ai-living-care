...
## 2026-08-06: 문서를 3계층으로 나눈다 — @ 자동로딩 / gateway / lazy

- 변경: 루트 `CLAUDE.md` 의 문서 참조를 `@`(매 세션 자동 로딩) 3개와 무표(lazy) 나머지로 분리.
  `docs/doc-map.md` 를 라우팅 정본으로 재편 — 「하려는 일 → 열 문서 / 안 여는 것」 fail-closed 표 신설.
  `README` · `harness.md` 를 같은 표기로 정렬. `docs/context/`·`docs/handoff/` 를 `REFERENCE-ONLY` 로 표시.
- 이유: **루트의 문서 참조 14개가 전부 `@` 였다.** 제목은 「필요할 때만 읽을 것」인데 실제로는
  spec 962줄 포함 **매 세션 2,003줄이 자동 로딩**되고 있었다. 하네스가 「962줄 통독 금지」라
  적어 둔 바로 그 파일을 루트가 강제로 열고 있었다. Lazy Loading 이 표기상으로만 존재했다.
- 결과: **2,003줄 → 302줄 (85% 감소).** `@` 는 doc-map · harness · status 셋뿐.
- 근거: 타 프로젝트 루트의 `@`/gateway/lazy 3계층 표기. 다만 그쪽이 루트에 담은 상태·버전은
  4인 병렬에서 충돌원이라 채택하지 않았다 (루트 동시 수정 충돌 3/4 실측).
