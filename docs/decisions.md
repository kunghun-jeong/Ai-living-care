...
## 2026-08-06: 상태 수확은 하되 **파일에 쓰지 않는다** (`make status`)

- 변경: `anchor.py --status` 신설 — 48개 `CLAUDE.md` 헤더의 `> **상태**` 줄을 읽어
  한 화면에 모아 출력한다. **어떤 파일도 쓰지 않는다.** `make status` 로 배선.
- 이유: 수확 결과를 커밋하는 세 방식을 4인 병렬로 측정했다 —
  평면 수확(doc-map 한 곳) **충돌 3/4** · 계층 수확(팀장 dir별) **1/4** · 미수확 **0/4**.
  생성물이라 재실행으로 풀리는 기계적 충돌이지만 **매 PR 마다** 난다. 마찰이 크면
  사람이 재생성을 건너뛰고 → CI 가 막고 → 마찰이 더 커지는 악순환이 된다.
  수확의 실익은 「한 화면에서 전체 현황」 하나뿐인데, 그건 **읽는 시점에 만들면 된다.**
- 결과: 충돌 0 · 낡을 수 없음(파일이 아니다) · F-12 재발 위험 0(쓰지 않는다) · CI 동기 검사 불필요.
- 손해: GitHub 웹에서는 표가 안 보인다. 필요해지면 CI 가 PR 코멘트로 붙이면 된다.
- 검증: `knowledge_graph` 상태 줄을 고치니 표가 즉시 따라왔다. 원복도 확인.

## 2026-08-06: 컴포넌트 헤더 48개를 4줄 규약으로 이관

- 변경: 모든 컴포넌트 `CLAUDE.md` 헤더를 `역할 / 상태 / 읽을 절 / 정본` 4줄로.
  담당자 소유 트리(`perception` `reasoning` `action` `limo-MCP` `limo-patrol-viz`)에는
  `⛔ 코드는 담당 연구원 소유 (D-17)` 를 헤더에 박았다.
- 「읽을 절」 효과: 지목 절 평균 **46줄** — 이전에는 헤더가 spec **962줄 전체**를 가리켰다.
  **컴포넌트 하나 열 때 spec 읽기량 95% 감소.** 없는 절을 지목한 건 0건(검증함).
- 「상태」는 기계가 읽는 줄이다 — `Phase · 진행 · 갭 · 작업`. 수확 장치가 붙으면
  이 줄이 `doc-map` §2 팀장 문서로 자동으로 올라간다.
- **절 지목이 틀렸으면 담당자가 고친다.** 이건 초안이다.

## 2026-08-06: 헤더 규약에서 위계 라벨을 뺀다 + 끊긴 하위에 CLAUDE.md

- 변경: 컴포넌트 헤더를 `역할 / 상태 / 읽을 절 / 정본` 4줄로. **위계 라벨(팀장·부장)을 쓰지 않는다.**
  `contracts/` 4종 · `docs/spec` · `docs/context` · `docs/handoff` 에 `CLAUDE.md` 신설.
  `docs/spec/CLAUDE.md` 는 절 색인(시작 줄·크기) — 헤더의 「읽을 절」이 가리키는 지도다.
- 이유: 위계 라벨을 문서에 박으면 **신규 컴포넌트가 생길 때마다 라벨이 밀려 전부 다시 써야 한다.**
  경로가 곧 위계이므로 라벨은 중복이고 낡음의 원천이다. 실증: `knowledge_graph/` 아래 컴포넌트를
  하나 넣었더니 `anchor.py` 가 부모 **본문** 한 줄만 요구했고 **헤더는 그대로**였다.
- 「읽을 절」: 헤더가 spec 962줄을 통째로 가리키던 것을 절 단위로. KG 의 경우 §3.1+§2.3 약 33줄.
- `contracts/` 는 팀이 곧 작업할 곳인데 위계가 끊겨 있었다 — 하위 4개에 `CLAUDE.md` 가 없어
  스키마 작업이 보고 체계 밖으로 나가고 있었다. 틀만 만들고 **필드는 비웠다** (설계는 팀 결정).
