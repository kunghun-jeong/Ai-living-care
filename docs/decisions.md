...
## 2026-08-06: 안전·실행 차단 결함 일괄 수정 (F-1~F-5, F-46~F-55)

- 변경: `Perceptions.py`(인코딩 분기·콜백 예외 격리·프레임 신선도·30프레임 링버퍼), `Actions.py`(늦은 수락 취소·완주 타임아웃 시 goal 취소·노드 시계·취소 사유 단일화·웨이포인트 검증·Nav2 부재 즉시 실패), `Reasonings.py`(증거 이미지 신뢰도 하한·정지한 스캔 결과 차단·`conclusive` 플래그), `MCP_server.py`(크롭 실패 노출·YOLO 워밍업 실패 노출·`Optional[float]`), `requirements.txt`(mcp>=2.0·numpy<2·pillow·opencv 선언), 순찰 도구(map.yaml 정본화·N_RAYS 48·PATROL 자기검사·TF base_link).
- 이유: 전부 **조용히 잘못 보고하는** 부류였다. 죽은 카메라가 「할머니 정상」을, conf 0.08 오탐이 「할머니 사진」을, 멈추지 않은 로봇이 「시퀀스 종료」를 만들고 있었다.
- 실패 경로 실행: `verify_fixes.py` 21건 통과 — 저신뢰 오탐 차단, 크롭 실패 노출, 정지 후 found 미변경, 카메라 정지 시 `conclusive:false`, 웨이포인트 8종 거부. `run_coverage.sh` 재실행 93.6% 동일. PATROL 좌표를 어긋내 자기검사 발동 확인.
- D-14/D-17: `limo-MCP/`·`limo-patrol-viz/` 내용 변경 — 구조·파일명·경로는 그대로.
...
> D-9 ~ D-17 전부 spec §0.2 에 반영돼 있다 (2026-08-06 확인).
