...
| — | **DA-11** | **감사기 자기 감사** — 장치·규칙이 실제로 실행됐는지, `SOT.md` §5 AR 표 = `sot_audit.py`가 돌린 규칙, 「DA-1~DA-N」 표기 |
...
> **장치를 하나 추가할 때마다 이 표에 행을 넣는다.** DA-11이 이 표와 `DEVICES`의 일치를 강제한다.
>
> **분모를 믿지 말 것.** 「N/N 통과」의 분모는 저장소 상태의 함수다 — 검사를 지우면 분자와 분모가 함께 줄어
> 언제나 100%가 된다. `sot_audit.py`의 `audit()` 본문을 비우면 **0/0 PASS · exit 0**이었다.
> DA-11이 그것을 막는다.
