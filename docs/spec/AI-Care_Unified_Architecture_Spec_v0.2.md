...
> **문서 위상**: **참조구현의 설계 정본**이자 **매거진 논문의 기준 문서**.
> 이 문서는 그중 **참조구현이 실제로 만드는 부분** — 제안서 ⑦의 「단일 엣지 왕복 → 다중 엣지 병렬 →
> 엣지 간 협력」 3단계 확장 경로, 곧 `D-5`의 Phase 0·2·3 — 을 설계한다.
> **참조구현이 만드는 것은 여기 먼저 정의한다** —
> 코드가 앞서면 이 문서를 고친다.
>
> 용어는 **§2가 정본**이며 코드·제안서·논문이 이를 인용한다. 원본 자료의 이표기는 부록 B에 남긴다.
>
...
| **D-9** *(SOT)* | A2A 종단점을 **최상위가 아니라 각 에이전트 안에** 둔다 — `manager_ai_agent/mcp_client/`(2026-08-18 `a2a_client/`로 개명, SOT D-20), `worker_ai_agent/mcp_server/` | `AI-Care_A2A_Core_Context` §4가 A2A Client를 Manager AI Agent에, A2A Server·Agent Executor를 Worker AI Agent에 배정. §10.1도 `MCP_server.py`를 WAC/A2A 종단점으로 매핑. **최상위 `a2a/`는 근거 없음** |
...
| **D-17** *(SOT)* | **소유 경계.** `worker_ai_agent/limo-MCP/**` 와 `tools/limo-patrol-viz/**` 의 **코드는 담당 연구원 소유**다. 다른 사람은 읽고 `docs/status.md` 에 결함을 기록할 뿐 고치지 않는다. 각 트리의 `CLAUDE.md` 만 예외 | 원본 보존(D-14)은 파일 배치 규칙이 아니라 **역할 경계**다. **보존 범위는 구조·파일명·경로이며 내용 동결이 아니다** — 내용까지 얼리면 Phase 0 작업 7건과 안전 결함 수정이 전부 막힌다. 담당자의 내용 변경은 결정 기록을 조건으로 허용 |
| **D-18** *(SOT)* | **안전 결함의 인계 경로.** 소유 경계 때문에 고칠 수 없는 안전 결함은 `docs/status.md` 「안전 — 담당자 통지」 표와 그 컴포넌트 `CLAUDE.md` 의 `상태` 줄에 **함께** 적는다 | 경계는 「고치지 마라」이지 「묻어라」가 아니다. `anchor.py` A5 가 두 자리의 짝을 양방향으로 강제한다 |
...
> **배치에 대한 단서**: 위 1번은 **참조구현의 배치**(Manager·Worker 동일 엣지)를 말한다.
> 제안서 ①이 정의하는 배치 모델은 **클라우드 관리 에이전트 – 엣지 작업 에이전트 – 이동체 3계층**이며,
> **참조구현 배치는 그 모델의 한 사례다.** Manager 가 클라우드에 놓이면 1번 논거는 그 배치에서
> 성립하지 않지만, 2·3번과 `P-6`(전송 독립성 — stdio / Streamable HTTP 를 배치에 따라 선택)이
> 바인딩을 그대로 지탱한다. **바인딩은 배치에 종속되지 않는다** — 그것이 P-6 을 둔 이유다.
...
> **권고**: Phase 3 초기는 **M-1로 시작**한다. P-5(감사 가능성)를 무료로 얻고, Worker에 Client 구현을 추가할 필요가 없다. M-2는 지연이 병목으로 실측된 뒤에 도입하고, 그때 **delegation token 데이터 모델이 표준화 항목 후보**가 된다 (**§11.1 S-1~S-7 에 미등재 — 번호 미정.** `S-5` 는 Intent Assurance / Worker Report Model 이다).
>
